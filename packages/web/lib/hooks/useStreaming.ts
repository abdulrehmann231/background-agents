...
/**
 * useStreaming - SSE streaming logic extracted from useChatWithSync
 *
 * Handles SSE connection, reconnection, and message updates for agent streaming.
 */
...
interface UseStreamingOptions {
  onConflictStateChange?: (state: { inRebase: boolean; inMerge: boolean; conflictedFiles: string[] }) => void
}
...
export function useStreaming(options: UseStreamingOptions = {}) {
  ...
  const isStreaming = useCallback((chatId: string) => {
    return useStreamStore.getState().isStreaming(chatId)
  }, [])
