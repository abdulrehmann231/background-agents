...
import { localHourToUtc, getTimezoneName } from "@/lib/scheduled-jobs/timezone"
import { inferIntervalMode, UNIT_MINUTES, type IntervalUnit } from "@/lib/scheduled-jobs/intervals"
...
/**
 * Owns all state and side effects for the scheduled-job create/edit form.
 * The form component stays presentational and wires this hook's values into
 * its sub-sections.
 */
export function useScheduledJobForm({ open, job, onClose, onSuccess }: UseScheduledJobFormArgs) {
  ...
  return {
    ...
    isCustomInterval, setIsCustomInterval,
    customIntervalValue, setCustomIntervalValue,
    customIntervalUnit, setCustomIntervalUnit,
    effectiveIntervalMinutes,
    runAtHourLocal, setRunAtHourLocal,
    runAtDay, setRunAtDay,
    timezoneName,
    // Webhook
    incomingToken,
    incomingWebhookUrl,
    ...
    // Options
    autoPR, setAutoPR,
    continueFromLastRun, setContinueFromLastRun,
    showContinueOption,
    showAutoPROption,
    hasOptions,
    // Actions
...
export type ScheduledJobFormState = ReturnType<typeof useScheduledJobForm>
